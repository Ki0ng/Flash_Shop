...
for(let i = 0; i < minusButtons.length; i++) {
    ...
    window.location = `${window.location.pathname}/removeFromCart?product_id=${product_id}`;
  })
}
...
for (let i = 0; i < minusButtons.length; i++) {
  ...
  minusButtons[i].addEventListener("click", () => {
    ...
    if (quantity > 1) {
      ...
      window.location = `${window.location.pathname}/updateCartItem?product_id=${product_id}&&value=${-1}&&price=${price}`;
    }
